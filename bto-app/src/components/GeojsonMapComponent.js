...
const GeojsonMapComponent = () => {
  ...
  const HomeSetPopup = ({ onClose }) => {
    return (
      <div className="popup-container" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 1000, background: 'white', padding: '20px', borderRadius: '5px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)' }}>
  ...
  const FormPopup = () => {
    return (
      <Box style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 1000, background: 'white', padding: '20px', borderRadius: '5px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)' }}>
        <InputLabel id="demo-simple-project-name" sx={{ color: "black", fontSize: "1.5rem", fontWeight: "bold", textAlign: "center" }}>Enter details: </InputLabel>
        <Box style={{ marginBottom: 20, marginTop: 10 }}>
          <InputLabel id="demo-simple-project-name">Project Name</InputLabel>
          <input style={{ width: "95%", height: 32, fontSize: "1rem" }} type="text" value={projectnameinform} onChange={handleProjectNameInForm} />
  ...
  const handleGeocode = async () => {
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        const road = response.data[0].address.road ? response.data[0].address.road : response.data[0].display_name;
  ...
  const handleReverseGeocode = async (latitude, longitude) => {
    try {
      ...
      if (response.data) {
        ...
        const road = response.data.address.road ? response.data.address.road : response.data.display_name;
        setHomeLocation({
          address: road,
          latitude: latitude,
          longitude: longitude
    ...
    <div className="map-container">
      <div className="map-container__top-bar">
        ...
        <div className="slider-container">
          ...
          {showDetails && (<div>
            <span style={{ fontWeight: "bold", fontSize: 18 }}>Project Name:  </span>{projectName}<br />
            <span style={{ fontWeight: "bold", fontSize: 18 }}>Number of Rooms:  </span>{numberOfRooms}
          </div>)}
        </div>
      </div>
                      ...
                      <tr key={index}>
                        <td>
                          ...
                          <div className="table-style__address">
                            <span>{extractAttributeName(marker, "ADDRESSSTREETNAME")}</span>
                            {extractAttributeName(marker, "ADDRESSPOSTALCODE")}
                            <span>{extractAttributeName(marker, "STREET_NAME")}</span>
      ...
      <div className="info-section">
        <p>For more information on upcoming BTOs, you can visit <a href="https://homes.hdb.gov.sg/home/landing" target="_blank" rel="noopener noreferrer">https://homes.hdb.gov.sg/home/landing</a></p>
      </div>

      <div>
        {showPopup && <HomeSetPopup onClose={closePopup} />}
      </div>
      <div>
